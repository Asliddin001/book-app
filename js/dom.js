...
function createBook(bookTitle, bookAuthor, bookYear, isCompleted) {
  ...
  author.innerHTML = `Author: <span>${bookAuthor}</span>`;

  const year = document.createElement("p");
  year.innerHTML = `Year: <span>${bookYear}</span>`;
  ...
  if (isCompleted) {
    ...
    buttons.append(createUncompletedButton(), createTrashButton());
  } else {
    buttons.classList.add("uncompleted");
    buttons.append(createCompletedButton(), createTrashButton());
  }
...
function createButton(buttonTypeClass, eventListener) {
  ...
  if (button.classList.contains("completed-button")) {
    ...
    button.setAttribute("title", "Mark as Uncompleted");
    ...
    button.setAttribute("title", "Delete Book");
  }
...
function addBookToCompleted(bookElement) {
  ...
  const bookAuthor = bookElement.querySelector(
    ".book-desc p:nth-child(2) span"
  ).innerText;
  const bookYear = bookElement.querySelector(
    ".book-desc p:nth-child(3) span"
  ).innerText;
